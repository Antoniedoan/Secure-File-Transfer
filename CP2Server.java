        ...
        try {
            ...
            while (!connectionSocket.isClosed()) {
                ...
                if (packetType.equals("filename")) {
                    ...
                    decryptor.init(Cipher.DECRYPT_MODE, sharedKey);
                    //decrypt
                    ...
                    toClient.flush();
                    System.out.println(enMsgInByte);

//                    FileInputStream CAInputStream = new FileInputStream(signedCertificateFile);
...
//                    deMsg.init(Cipher.DECRYPT_MODE, publicKey);
//                    byte[] decryptedResponse = deMsg.doFinal(enMsgInByte);
//                    System.out.println(decryptedResponse);
                    ...
                    System.out.println(new String(encryptedNonce));

                }else if(packetType.equals("aes")){
                    System.out.println("Receiving AES Key");
                    int enKeyLength = fromClient.readInt();
                    byte[] enKey = new byte[enKeyLength];
                    fromClient.readFully(enKey);

                    // set private key to encrypt
                    Cipher decryptor = Cipher.getInstance(hashAlgo);
                    decryptor.init(Cipher.DECRYPT_MODE, privateKey);

                    byte[] deKey = decryptor.doFinal(enKey);
                    sharedKey = new SecretKeySpec(deKey,0,deKey.length,"AES");
                    System.out.println("sharedKey: " + sharedKey.toString());
                }
    ...
    private static void sendFile(DataOutputStream toClient, String filename, String filepath) throws Exception{
        ...
        System.out.println("Cert successfully sent.");
